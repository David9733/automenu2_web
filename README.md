...
### 2. 모바일 텍스트 컨테이너 오버플로우

**문제:** 그리드 상자는 제대로 표시됐으나 내부 텍스트가 상자 밖으로 벗어남<br>
**원인:** 데스크톱 기준의 고정 폰트 크기가 좁은 화면에서 줄바꿈되거나 컨테이너를 초과<br>
**해결:** Tailwind 반응형 텍스트 크기 클래스와 `whitespace-nowrap`으로 텍스트가 컨테이너 내에 맞게 조정<br>
- `text-base sm:text-xl lg:text-2xl` 적용으로 화면 너비에 따라 폰트 크기 단계적 축소
