...
- **모바일 그리드 레이아웃 깨짐** : 데스크톱 기준으로만 설계하여 모바일에서 확인했을 때 레이아웃이 화면을 벗어나거나<br>카드가 너무 작게 표시됐습니다. Features는 1열→2열(md)→3열(lg), Screenshots는 2열→3열(sm)→5열(lg)로 Tailwind<br>브레이크포인트에 따라 열 수를 조정하고 aspect-[9/16]으로 비율을 고정하여 해결했습니다. 이 경험으로 반응형 설계는 처음부터 모바일 기준으로 설계하고 넓혀가는 모바일 퍼스트 접근이 훨씬 효율적이라는 것을 체감했습니다.
