...
- **모바일 그리드 레이아웃 깨짐** : 데스크톱 기준(3열 그리드)으로만 설계하여 모바일에서 확인했을 때 레이아웃이 화면을<br>벗어나거나 카드가 너무 작게 표시됐습니다. Features는 1열→2열(md)→3열(lg), Screenshots는 2열→3열(sm)→5열(lg)로 Tailwind 브레이크포인트에 따라 열 수를 단계적으로 조정하고 aspect-[9/16]으로 비율을 고정하여 해결했습니다. 이 경험으로 반응형 설계는 완성 후 수정하는 것보다 처음부터 모바일 기준으로 설계하고 넓혀가는 모바일 퍼스트 접근이 훨씬 효율적이라는 것을 체감했습니다.
  
- **모바일 텍스트 컨테이너 오버플로우** : 그리드 상자는 정상이었으나 내부 텍스트가 컨테이너 밖으로 벗어났습니다. 데스크톱 기준 고정 폰트 크기가 좁은 화면에서 줄바꿈되거나 컨테이너를 초과하는 것이 원인이었습니다. text-base sm:text-xl lg:text-2xl 클래스를 적용해 화면 너비에 따라 폰트 크기를 단계적으로 축소하여 해결했습니다. 레이아웃과 텍스트는<br>별개로 깨질 수 있다는 것을 알게 됐고, 컨테이너뿐 아니라 내부 요소도 브레이크포인트별로 함께 검토해야 한다는 점을<br>깨달았습니다.
