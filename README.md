...
- **모바일 텍스트 컨테이너 오버플로우** : 그리드 상자는 정상이었으나 내부 텍스트가 컨테이너 밖으로 벗어났습니다. 데스크톱 기준 고정 폰트 크기가 좁은 화면에서 줄바꿈되거나 컨테이너를 초과하는 것이 원인이었습니다. text-base sm:text-xl lg:text-2xl 클래스를 적용해 화면 너비에 따라 폰트 크기를 축소하여 해결했습니다. 레이아웃과 텍스트는 별개로 깨질 수 있다는 것을 알게 됐고, 컨테이너뿐 아니라 내부 요소도 브레이크포인트별로 함께 검토해야 한다는 점을<br>깨달았습니다.
